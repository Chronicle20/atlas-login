...
func Provider[A any, M any](l logrus.FieldLogger, span opentracing.Span, tenant tenant.Model) func(r Request[A], t Transformer[A, M]) model.Provider[M] {
	return func(r Request[A], t Transformer[A, M]) model.Provider[M] {
		return func() (M, error) {
			var result M
			resp, err := r(l, span, tenant)
			if err != nil {
				return result, err
			}
